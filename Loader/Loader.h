 ...
 *
 * We call this a Loader because it calls function of Naive Directory
 * Explorer and loads all the files matching the ragexes and carries
 ...
 * ========================
 * Loader ld(path);
 * ld.addPattern(std::string patt);
 * ld.find(std::string path);
 * ld.match_regexes(std::vector<std::string> regexes);
 * ld.doFile(dir,filename);
 * ld.doDir(dirname);
 * 
 *
 * Required Files:
